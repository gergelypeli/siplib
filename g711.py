# Disclaimer: tables copied from the Sox source.
#
...
lsx_14linear2ulaw = [
   ...
   0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
   0x80, 0x80, 0x80, 0x80,
   0x80  # for rounding overflow
]


def encode_pcmu_sample(s16):
    return lsx_14linear2ulaw[(((s16 + 2) >> 2) + 0x2000)]


def encode_pcma_sample(s16):
    return lsx_13linear2alaw[(((s16 + 4) >> 3) + 0x1000)]
    
    
def decode_pcmu_sample(u8):
    return lsx_ulaw2linear16[u8]


def decode_pcma_sample(u8):
    return lsx_alaw2linear16[u8]
...
def encode_pcmu(x):
    ...
    for i in range(n):
        y[i] = encode_pcmu_sample(struct.unpack_from("<h", x, 2 * i)[0])
...
def encode_pcma(x):
    ...
    for i in range(n):
        y[i] = encode_pcma_sample(struct.unpack_from("<h", x, 2 * i)[0])
...
def decode_pcmu(x):
    ...
    for i in range(n):
        struct.pack_into("<h", y, 2 * i, decode_pcmu_sample(x[i]))
...
def decode_pcma(x):
    ...
    for i in range(n):
        struct.pack_into("<h", y, 2 * i, decode_pcma_sample(x[i]))
